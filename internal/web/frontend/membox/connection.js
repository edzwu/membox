...
export function renderConnectionButton() {
  ...
  connectionButton.dataset.connected = String(session.connected);
...
async function toggleConnection() {
  ...
  if (session.connected && session.annotationsDirty) {
    ...
    showToast(session.connected ? 'Connected to membox — arrow now syncs' : 'Could not connect to membox');
  }
}
