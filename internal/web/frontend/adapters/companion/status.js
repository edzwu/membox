/* Bottom-left status cluster: save/copy badge and the add-related button.
   The badge's hollow dot means there are changes to save; a filled dot means
   the document is durable and the badge copies its UUID.
...
function createStatusBadge() {
  ...
  badge.addEventListener('click', () => {
    if (!session.connected || session.syncing || session.saveInFlight) return;
...
export function initStatus() {
  ...
  // Keep the disconnected status area absent across empty/reading transitions.
