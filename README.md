...
The TUI opens in modified-time order: the newest documents come first, and documents without a timestamp last. Markdown and PDF files share one tree. Press `ctrl+p` to cycle its media scope through all documents → Markdown → PDF → images; a full-word status badge is shown whenever the scope is not all. The image scope is ready for image documents once image ingest lands. `ctrl+r` reloads the catalog immediately before running a full path scan, then focuses the newest visible document added since the previous tree snapshot—so a PDF imported by Miru appears without waiting for the scan. Outside the filter input, press `s` to toggle back to filename (dictionary) order; changing the sort focuses the new first row. In the tree view, Home and End jump to the first and last rows.
