...
The connection icon in the Miru top bar shows whether the reader is connected to membox; click it to toggle the connection. While connected, dropping one PDF onto Miru uploads it to the remembered managed PDF directory (by default `~/Documents/membox-pdfs`) and catalogs it without replacing the Markdown currently being read; uploads are limited to 500 MiB. The lower-right Markdown arrow syncs both Markdown and annotations to membox (updating the current UUID, or creating a note when the paste is new). While disconnected, the same arrow keeps Miru's normal local download behavior. Every synced annotation is an individual `*-note.md` document: its blockquote/explanation is Markdown content, while SQLite stores the target UUID and anchoring hints. Pasting clipboard images into the selection Note composer uploads validated raster bytes to the content-addressed private store at `$MEMBOX_HOME/assets/note-images/` (outside Markdown/Git) and inserts only `![](/api/note-assets/<sha>.<ext>)` into the note; Miru serves and renders that image inline. Miru assembles its annotation JSON only when the document is opened; it is not a second persisted note copy. Reading progress is separate DB UI state, so scrolling never rewrites note files. Notes and progress auto-save in the background for any document bound to membox; taking the first note on pasted (not yet synced) content automatically creates the membox source document while connected. The explicit arrow sync additionally writes the Markdown source.

Miru has two translation modes. Selecting a passage and choosing `译` streams a Chinese translation inline, then persists the completed result through the same anchored-note path as Q&A and selection summaries (`kind: "translation"` in an independent `*-note.md`); reopening the source restores the translation automatically without changing the source Markdown. Anchors may nest, so a translated paragraph can still contain sentence-level highlights, notes, Q&A, summaries, or further translations without replacing the paragraph artifact. Clicking that translated passage exposes `译` again to regenerate and update it. `Cmd/Ctrl+Alt+T` remains disposable full-document immersive translation: Miru sends one rendered prose block at a time, starting near the current viewport, and removes those transient blocks when toggled off or when another document opens. Web Companion forwards NDJSON over mmd's owner-only Unix socket, and mmd runs an isolated tool-free `pi --mode rpc` turn with local `qwen3:14b`. Completed model outputs are also cached in a standalone, disposable database (`~/.membox/mmd/translation-cache.db`, outside the core catalog) keyed by normalized source text + target language + model + mode + prompt version. That database is only a computation cache: deleting it does not remove persisted selection translations, while repeated generation can replay instantly without starting Pi. This requires local Ollama with `qwen3:14b` pulled. mmd starts on demand and stops itself after 30 minutes without work.
